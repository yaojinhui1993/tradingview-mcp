...
tv data download/lines/labels/tables/boxes/strategy/trades/equity/depth/indicator
...
| `chart_download_data` | Use TradingView's Download chart data dialog to generate CSV with symbol + indicator columns. Captures the CSV Blob in the background when possible, then falls back to native download/mouse events | File path + summary; CSV on disk |
